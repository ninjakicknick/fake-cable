...
export async function fetchStationChannel(value){
  const res=await fetch('/api/channels',{signal:AbortSignal.timeout(60000),method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({channels:[value]})});
...
export async function searchStationChannels(query){
  const res=await fetch('/api/channels',{signal:AbortSignal.timeout(60000),method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'search',query})});
  const data=await res.json();
