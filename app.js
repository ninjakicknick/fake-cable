import {DEFAULT_STATIONS,DEFAULT_COMMERCIAL_PLAYLIST,DEMO_CHANNELS} from './catalog.js';
...
import {isDirectYouTubeInput,stationLabelAfterFetch,fetchStationChannel,searchStationChannels,channelFromApi} from './channel-input.js';
import {createGuideController} from './guide.js';
import {CATALOG_VERSION,LINEUP_VERSION,STORAGE_KEYS,createSafeStorage,loadStations,loadCommercialConfig,loadLineupOrder,loadMixes,loadSourceChannels,saveLineupState} from './storage.js';
...
const storage=createSafeStorage(()=>window.localStorage,()=>toast('BROWSER STORAGE UNAVAILABLE — CHANGES MAY NOT SURVIVE RELOAD'));
let SOURCE_CHANNELS=loadSourceChannels(storage,DEMO_CHANNELS);
let CHANNELS=[];
let mixes=loadMixes(storage);
...
let {config:commercialConfig,shouldLoadDefault:shouldLoadDefaultCommercials}=loadCommercialConfig(storage,commercialDefaults);
...
const reconcileLineup=createLineupScheduler();
function rememberChannel(){if(!state.current)return;storage.setItem(STORAGE_KEYS.lastChannel,String(state.current.row));storage.setItem(STORAGE_KEYS.lastChannelId,state.current.ch.channelId)}
function scheduleChannels(previous=CHANNELS){
 const changed=reconcileLineup(state,previous,CHANNELS,activeCommercials());
 rememberChannel();
 if(changed){
  if(state.current)loadCurrentProgram();
  else {state.player?.stopVideo?.();hideTuningStatic();}
 }
}
function normalizeLineup(){
 const previous=CHANNELS;
 const composed=composeMixLineup(SOURCE_CHANNELS,mixes,lineupOrder);
 SOURCE_CHANNELS=composed.sources;mixes=composed.mixes;CHANNELS=composed.channels;lineupOrder=composed.order;
 scheduleChannels(previous);
}
...
const captions=createCaptionController({state,storage,broadcast:broadcastRemoteStatus,toast});
...
function showBanner(){if(!state.current)return;const {ch,p}=state.current,b=document.querySelector('#banner');clearTimeout(state.bannerTimer);document.querySelector('#banner-channel').textContent=`CH ${String(ch.n).padStart(2,'0')}  ·  ${ch.name}`;if(p.isCommercial){const index=ch.schedule.indexOf(p),next=ch.schedule.slice(index+1).find(item=>!item.isCommercial);if(!next){b.classList.remove('show');broadcastRemoteStatus();return}const startsIn=Math.max(0,next.start-nowSec()),minutes=Math.ceil(startsIn/60);document.querySelector('#banner-title').textContent=`UP NEXT: ${next.title}`;document.querySelector('#banner-meta').textContent=minutes<=1?'STARTS IN 1 MINUTE':`STARTS IN ${minutes} MINUTES`;document.querySelector('#banner-progress').style.width='0%'}else{const elapsed=Math.max(0,nowSec()-p.start);document.querySelector('#banner-title').textContent=p.title;document.querySelector('#banner-meta').textContent=`${p.source}  ·  ${Math.floor(elapsed/60)} MINUTES INTO PROGRAM`;document.querySelector('#banner-progress').style.width=Math.min(100,elapsed/p.duration*100)+'%'}b.classList.add('show');state.bannerTimer=setTimeout(()=>b.classList.remove('show'),4200);broadcastRemoteStatus()}
...
function performAction(action){
 ...
 scheduleRemoteHide();
 if(action==='guide'){if(!state.guide&&state.current){state.row=state.current.row;state.col=currentIndex(state.current.ch);state.guideFollowingLive=true}showGuide(!state.guide);}
 ...
 else if(action==='captions')captions.toggle();
...
function saveCommercialConfig(){storage.setItem('fake-cable-commercials',JSON.stringify(commercialConfig))}
...
function moveGuideChannel(index,direction){const target=index+direction;if(stationUpdating||target<0||target>=CHANNELS.length)return;const moved=CHANNELS[index];[lineupOrder[index],lineupOrder[target]]=[lineupOrder[target],lineupOrder[index]];normalizeLineup();saveLineup();renderStationPicker();renderMixes();if(state.guide)render();broadcastRemoteStatus();setupStatus(`${moved.name} moved to channel ${target+2}.`)}
...
function renameGuideChannel(index){if(stationUpdating)return;const channel=CHANNELS[index];if(!channel)return;if(channel.isMix){const mix=mixes.find(item=>item.id===channel.mixId),answer=window.prompt('Rename this mix channel.',mix?.name||channel.name);if(answer===null)return;const name=answer.trim();if(!name){setupStatus('A mix channel needs a name.');return}mix.name=name;normalizeLineup();saveLineup();renderStationPicker();renderMixes();if(state.guide)render();broadcastRemoteStatus();setupStatus(`Channel renamed to ${name}.`);return}const sourceIndex=SOURCE_CHANNELS.findIndex(item=>item.channelId===channel.channelId);renameStation(sourceIndex)}
...
function saveMix(){if(stationUpdating)return;const name=document.querySelector('#mix-name').value.trim(),sourceIds=[...document.querySelectorAll('#mix-source-list input:checked')].map(input=>input.value),error=document.querySelector('#mix-error');if(!name){error.textContent='Give the channel a name.';return}if(sourceIds.length<2){error.textContent='Choose at least two source stations.';return}const mix={id:editingMixId||`mix:${Date.now().toString(36)}`,name,sourceIds,hideSources:document.querySelector('#mix-hide-sources').checked};if(editingMixId)mixes=mixes.map(item=>item.id===editingMixId?mix:item);else mixes.push(mix);normalizeLineup();saveLineup();state.row=Math.min(state.row,CHANNELS.length-1);closeMixEditor();renderStationPicker();renderMixes();if(state.guide)render();broadcastRemoteStatus()}
function saveLineup(){saveLineupState(storage,{stationPicker,sourceChannels:SOURCE_CHANNELS,mixes,lineupOrder});document.querySelector('#lineup-status').textContent=`${CHANNELS.length} stations`}
...
function startTelevision(){if(document.querySelector('#welcome').style.display==='none')return;showTuningStatic(800);document.querySelector('#welcome').style.display='none';const saved=Number(storage.getItem(STORAGE_KEYS.lastChannel)||0),last=Number.isInteger(saved)?saved:0;const savedRow=CHANNELS.findIndex(ch=>ch.channelId===storage.getItem(STORAGE_KEYS.lastChannelId));state.row=savedRow>=0?savedRow:Math.min(CHANNELS.length-1,Math.max(0,last));render();tune(state.row);scheduleRemoteHide()}
...
document.querySelector('#mix-list').addEventListener('click',e=>{const edit=e.target.closest('[data-edit-mix]'),remove=e.target.closest('[data-remove-mix]');if(edit){openMixEditor(edit.dataset.editMix);return}if(!remove||stationUpdating)return;mixes=mixes.filter(mix=>mix.id!==remove.dataset.removeMix);normalizeLineup();saveLineup();state.row=Math.min(state.row,CHANNELS.length-1);renderMixes();if(state.guide)render();broadcastRemoteStatus()});
document.addEventListener('keydown',e=>{if(isPhoneRemote)return;if(document.querySelector('#welcome').style.display!=='none'){if(e.key==='Enter')document.querySelector('#start').click();return}if(document.querySelector('#setup').style.display==='block'){if(e.key==='Enter'&&document.activeElement===document.querySelector('#channel-search'))searchChannel();return}if(e.target.closest?.('input,textarea,select,button,[contenteditable="true"],[role="dialog"]'))return;const k=e.key.toLowerCase();if(['arrowup','arrowdown','arrowleft','arrowright',' ','enter'].includes(k))e.preventDefault();if(k==='g'||k==='escape')performAction('guide');else if(k==='f')performAction('fullscreen');else if(k==='m')performAction('mute');else if(state.guide){if(k==='arrowup')performAction('nav-up');if(k==='arrowdown')performAction('nav-down');if(k==='arrowleft')performAction('nav-left');if(k==='arrowright')performAction('nav-right');if(k==='enter')performAction('watch')}else{if(k==='arrowup')performAction('up');if(k==='arrowdown')performAction('down');if(k==='enter'||k===' ')performAction('banner')}});
...
if(!isPhoneRemote&&stationPicker.length&&storage.getItem('fake-cable-lineup-version')!==LINEUP_VERSION){document.querySelector('#lineup-status').textContent='Adding new starter channels…';buildChannels('Your lineup has been expanded.')}
else if(!isPhoneRemote&&stationPicker.length&&storage.getItem(STORAGE_KEYS.catalogVersion)!==CATALOG_VERSION){document.querySelector('#lineup-status').textContent='Refreshing video filters…';buildChannels('Your stations have refreshed video filters.')}
